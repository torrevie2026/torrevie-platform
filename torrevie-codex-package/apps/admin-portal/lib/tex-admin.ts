...
export type TexBillingOverviewRecord = {
  tenant_id: string;
  tenant_name: string;
...
export async function listTexBillingOverview(
  client: SupabaseClient
): Promise<TexBillingOverviewRecord[]> {
  ...
  return [...tenantIds]
    .map((tenantId) => {
      ...
      return {
        ...
        stripe_customer_id: customer?.stripe_customer_id ?? subscription?.stripe_customer_id ?? "",
        stripe_subscription_id: subscription?.stripe_subscription_id ?? "",
        stripe_price_id: subscription?.stripe_price_id ?? "",
        plan_key: subscription?.plan_key ?? null,
        subscription_currency: subscription?.currency ?? "",
        stripe_status: subscription?.stripe_status ?? "not_connected",
